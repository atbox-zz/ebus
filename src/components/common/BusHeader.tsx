    ...
    <Header>
      ...
      <ul className="flex gap-5 items-center md:gap-14">
        ...
        <li className="text_link hidden md:block md:text-base">
          <NavLink to="https://ebus.tycg.gov.tw/ebus/route-search/%E6%A1%83%E5%9C%92%E5%8D%80%E6%A1%83%E5%9C%92%E7%B8%BD%E7%AB%99-24.9865803156191-121.313968000767" onClick={resetContext}
            className={({ isActive }) => isActive ? 'active' : ''}>
            {t('realtime1')}
          </NavLink>
        </li>        
        <li className="text_link hidden md:block md:text-base">
          <NavLink to="https://ebus.tycg.gov.tw/ebus/route-search/%E6%A1%83%E5%9C%92%E5%8D%80%E6%A1%83%E5%9C%92%E5%85%AC%E8%BB%8A%E7%AB%99-24.99024-121.312041" onClick={resetContext}
            className={({ isActive }) => isActive ? 'active' : ''}>
            {t('realtime1')}
          </NavLink>
        </li>
        */}
        <li className="text_link hidden md:block md:text-base">
          <NavLink to="/searchBus" onClick={resetContext}
            className={({ isActive }) => isActive ? 'active' : ''}>
            {t('road_bus')}
          </NavLink>
        </li>
        <li className="text_link hidden md:block md:text-base">
          <NavLink to="/searchStop" onClick={resetContext}
            className={({ isActive }) => isActive ? 'active' : ''}>
            {t('search_stop')}
          </NavLink>
        </li>
        <li className="text_link hidden md:block md:text-base">
          <NavLink to="/nearbyStop" onClick={resetContext}
            className={({ isActive }) => isActive ? 'active' : ''}>
            {t('nearby_stop')}
          </NavLink>
        </li>
...
function HeaderLogo({ goPage }: HeaderLogoProps) {
  ...
  return (
    <div className="text_hover flex gap-3 items-center bg-gray-500" onClick={() => goPage('')}>
